-- 1. Create an updatable view that includes customerNumber, customerName, contactLastName, and contactFirstName from the customers table. 
...
-- 2. Create a read-only view that joins the orderdetails table and the products table on productCode and includes orderNumber, productName, and quantityOrdered. 
...
-- 3. Write a query that uses an inline view to get the total number of orders for each customer. 
...
-- 4. The task here would be to create a stored procedure that creates a new table with productName and 
-- totalQuantityOrdered (this total should be aggregated from the orderdetails table). 
-- Then, create an AFTER INSERT trigger on the orderdetails table that calls this stored procedure to 
-- update the table (acting as a materialized view) whenever a new order detail is inserted.
